...
'''
Get the relation between motif and jieba and textrank4zh
'''
import os
...
#		print '300'
		...
		return [MK3,KMt3,MKz3,KMtz3],[Mword,keyword,keywordz]
	else:
...
#		print '400'
		...
		return [MK4,KMt4,MKz4,KMtz4],[Mword,keyword,keywordz]
	else:
		return [0.0,0.0,0.0,0.0],[[],[],[]]
...
def getresult(elem):
	...
	for m in xrange(4,M):
		...
		for i in xrange(T):
			...
			tt2=time.clock()
			tt=tt2-tt1
			l3,kw3=getratio300(elem,network_dic,tt)
			l4,kw4=getratio400(elem,network_dic,tt)		
...
def main():
	fo=open(path1+'findm4jz_1.txt','w')
...
#	if '1107' in ele:
		...
		i=0
		for elem in data:
			li,kw=getresult(elem)
			li=[str(el) for el in li]
			print ' '.join(li)
			fo.write('\001'.join(li)+'\001'+'m3'.join(kw[0])+'\001'+'j3'.join(kw[1])+'\001'+'z3'.join(kw[2])+'\001'+'m4'.join(kw[3])+'\001'+'j4'.join(kw[4])+'\001'+'z4'.join(kw[5])+'\n')
			print i
			i += 1
	fo.close()
